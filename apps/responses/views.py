...
class OpenResponseOpenCreateView(CreateView):
    ...
    def form_valid(self, form):
        messages.add_message(
            self.request, messages.SUCCESS, "Questionário enviado com sucessso"
        )
        return super().form_valid(form)

    def form_invalid(self, form):
        messages.add_message(
            self.request, messages.ERROR, "Não foi possível enviar o questionário"
        )
...
class RegionalResponseCreateView(
    ...
    def form_valid(self, form):
        form.interviewer = self.request.user.username
        messages.add_message(
            self.request, messages.SUCCESS, "Questionário enviado com sucessso"
        )
        return super().form_valid(form)
    ...
    def get_success_url(self, *args, **kwargs):
        return reverse_lazy(
            "responses:regional_create", kwargs={"pk": self.kwargs["pk"]}
        )
