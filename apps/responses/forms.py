...
class RegionalResponseForm(ResponseForm):
    ...
    def __init__(self, survey, user, *args, **kwargs):
        ...
        neighborhoods = Neighborhood.objects.select_related('sector', 'sector__city').filter(sector__city=city)

        if user.groups.filter(name='Entrevistadores').exists():
            sectors_list = user.surveys.all().values_list('regionalsurveyinterviewer__sectors', flat=True)
        ...
        if sector and neighborhood:
            if sector.n_responses:
                ...
                if count >= meta['sectors'][str(sector.pk)]['total']:
                    self.add_error('sector', 'Esta setor já preencheu todos os questionários')

            if neighborhood not in sector.neighborhoods.all():
                self.add_error('neighborhood', 'Este bairro não pertence ao setor selecionado')

        if neighborhood and not sector.n_responses:
            count = responses.filter(neighborhood=neighborhood).count()
            try:
                if count >= meta['sectors'][str(sector.pk)]['neighborhoods'][str(neighborhood.pk)]['surveys']:
                    self.add_error('neighborhood', 'Este bairro já preencheu todos os questionários')
            except KeyError:
                self.add_error('neighborhood', 'Este bairro não consta nas metas para esta pesquisa')

        if gender and sector:
            count = responses.filter(gender=gender, sector=sector).count()
            if count >= meta['sectors'][str(sector.pk)]['genders'][str(gender)]['surveys']:
                self.add_error('gender', 'Este sexo já preencheu todos os questionários')

        if age_group and sector:
            count = responses.filter(age_group=age_group, sector=sector).count()
            if count >= meta['sectors'][str(sector.pk)]['age_groups'][str(age_group)]['surveys']:
                self.add_error('age_group', 'Esta faixa etária já preencheu todos os questionários neste setor')
