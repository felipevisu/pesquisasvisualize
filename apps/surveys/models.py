...
class Survey(models.Model):
    ...
    client = models.ForeignKey('clients.Client', on_delete=models.SET_NULL, null=True, blank=True, related_name='surveys', verbose_name='cliente')
    city = models.ForeignKey(City, related_name='surveys', on_delete=models.SET_NULL, null=True, blank=True, verbose_name='cidade')
...
class OpenSurvey(Survey):
    max_responses = models.PositiveIntegerField('número de formulários', null=True, blank=True)
    ...
    bg_color = models.CharField('cor do fundo', max_length=50, null=True, blank=True, default="#007BFF")
...
class Question(models.Model):
    ...
    survey = models.ForeignKey(Survey, related_name='questions', on_delete=models.CASCADE, verbose_name='pesquisa')
    ...
    question_type = models.CharField('tipo de questão', max_length=25, choices=TYPES, default=RADIO)
    chart_type = models.CharField('tipo de gráfico', max_length=25, choices=CHART, default=PIE)
    choices = models.TextField('opções', blank=True, null=True, help_text="Utilize ';' para separar as opções")
...
class SurveyInterviewer(models.Model):
    interviewer = models.ForeignKey('accounts.User', related_name='surveys', verbose_name='entrevistadora', on_delete=models.CASCADE)
    survey = models.ForeignKey(Survey, related_name='interviewers', verbose_name='pesquisa', on_delete=models.CASCADE)
    creation_date = models.DateTimeField('data de criação', auto_now_add=True)


class RegionalSurveyInterviewer(SurveyInterviewer):
    sectors = models.ManyToManyField('locations.Sector', verbose_name='setores', blank=True)
