...
from django.views.generic import DeleteView, DetailView, ListView, TemplateView, UpdateView, FormView
...
from .filters import RegionalResponseFilter, OpenResponseFilter
from .forms import *
from .models import *
...
class SurveyListView(LoginRequiredMixin, PermissionRequiredMixin, ListView):
    ...
    def get_queryset(self):
        ...
        queryset = Survey.objects.all().select_related(
            'client', 'city', 'regionalsurvey', 'opensurvey'
        ).prefetch_related(
            'responses',
        ...
        if user.groups.filter(name='Entrevistadores').exists():
            ...
            queryset = queryset.filter(pk__in=u_surveys)
        if user.client:
...
class OpenSurveyCreateView(LoginRequiredMixin, NamedFormsetsMixin, PermissionRequiredMixin, SuccessURLMixin, CreateWithInlinesView):
    model = OpenSurvey
    inlines = [QuestionInline]
    inlines_names = ['Questions']
    fields = ['name', 'slug', 'client', 'city', 'header', 'bg_color', 'max_responses', 'open_form']
...
class OpenSurveyUpdateView(LoginRequiredMixin, NamedFormsetsMixin, PermissionRequiredMixin, SuccessURLMixin, UpdateWithInlinesView):
    model = OpenSurvey
    inlines = [QuestionInline]
    inlines_names = ['Questions']
    fields = ['name', 'slug', 'client', 'city', 'header', 'bg_color', 'max_responses', 'open_form']
...
class RegionalSurveyCreateView(LoginRequiredMixin, NamedFormsetsMixin, PermissionRequiredMixin, SuccessURLMixin, CreateWithInlinesView):
    model = RegionalSurvey
    inlines = [QuestionInline]
    inlines_names = ['Questions']
    fields = ['name', 'client', 'city', 'no_neigh']
    permission_required = ['surveys.add_survey']
...
class RegionalSurveyUpdateView(LoginRequiredMixin, NamedFormsetsMixin, PermissionRequiredMixin, SuccessURLMixin, UpdateWithInlinesView):
    model = RegionalSurvey
    inlines = [QuestionInline, RegionalSurveyInterviewerInline]
    inlines_names = ['Questions', 'Interviewers']
    fields = ['name', 'client', 'city', 'no_neigh', 'finalized']
    permission_required = ['surveys.change_survey']
...
class RegionalSurveyMetaView(LoginRequiredMixin, PermissionRequiredMixin, DetailView):
    ...
    def get_context_data(self, **kwargs):
        ...
        meta = survey.meta
        responses = list(RegionalResponse.objects.filter(survey=survey).values('sector', 'neighborhood', 'gender', 'age_group'))
        sectors = survey.city.sectors.values_list('pk', flat=True)
        
        if user.groups.filter(name='Entrevistadores').exists():
            sectors = user.surveys.filter(survey=survey).values_list('regionalsurveyinterviewer__sectors', flat=True).distinct()
            print(sectors)
            del_sectors = []
        ...
        for sector in meta['sectors']:
            for gender in meta['sectors'][sector]['genders']:
                meta['sectors'][sector]['genders'][gender]['responses'] = len([response for response in responses if response['sector'] == int(sector) and response['gender'] == int(gender)])
            for age_group in meta['sectors'][sector]['age_groups']:
                meta['sectors'][sector]['age_groups'][age_group]['responses'] = len([response for response in responses if response['sector'] == int(sector) and response['age_group'] == int(age_group)])
            for neigh in meta['sectors'][sector]['neighborhoods']:
                meta['sectors'][sector]['neighborhoods'][neigh]['responses'] = len([response for response in responses if response['neighborhood'] == int(neigh)])

        context['meta'] = meta
        return context


class SurveyOrderedView(LoginRequiredMixin, PermissionRequiredMixin, UpdateWithInlinesView):
    model = Survey
...
class MetaUpdateView(LoginRequiredMixin, PermissionRequiredMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        survey.save()
        messages.add_message(self.request, messages.SUCCESS, 'Metas atualizadas com sucessso')
...
class RegionalSurveyResultView(LoginRequiredMixin, PermissionRequiredMixin, FilterView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['answers'] = list(Answer.objects.filter(response__in=queryset).values('question', 'body', 'response'))
...
class QuestionModifyView(LoginRequiredMixin, PermissionRequiredMixin, FormView):
    ...
    def get_context_data(self, **kwargs):
        ...
        context['answers'] = list(question.answers.values('question', 'body', 'response'))
        return context
    ...
    def form_valid(self, form):
        ...
        if answers.count() > 0:
            messages.add_message(self.request, messages.SUCCESS, 'Respostas alteradas com sucesso')
        else:
            messages.add_message(self.request, messages.WARNING, 'Nenhuma resposta continha as palavras informadas')
...
class QuestionSectorView(LoginRequiredMixin, FilterView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        answers = list(Answer.objects.filter(response__in=filter.qs).values('question', 'body', 'response'))
...
def export_survey_xls(request, pk):
    ...
    response['Content-Disposition'] = 'attachment; filename="{}.xls"'.format(survey.city.name)
    ...
    row = ["Setor", "Bairro", "Gênero", "Idade"] + [question.text for question in survey.questions.all()]
    for col in range(len(row)):
        ws.write(index, col, row[col], font_style)

    responses = RegionalResponse.objects.filter(survey=survey).select_related('sector', 'neighborhood').prefetch_related('answers')
    for res in responses:
        row = [res.sector.name, res.neighborhood.name if res.neighborhood else '', res.get_gender_display(), res.get_age_group_display()]
