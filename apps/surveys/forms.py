...
class QuestionForm(forms.ModelForm):
    class Meta:
        model = Question
        fields = ['text', 'required', 'question_type', 'chart_type', 'choices', 'key', 'order']
        widgets = {
...
class RegionalSurveyInterviewerForm(forms.ModelForm):
    interviewer = forms.ModelChoiceField(queryset=User.objects.filter(groups__name='Entrevistadores'), label='Entrevistador(a)')
    sectors = forms.ModelMultipleChoiceField(queryset=Sector.objects.all(), label='Setores')
...
class SurveyInterviewerForm(forms.ModelForm):
    interviewer = forms.ModelChoiceField(queryset=User.objects.all(), label='Entrevistador(a)')
